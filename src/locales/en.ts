export const enTranslation = {
  ...
  login: {
    ...
    magicStringPlaceholder: 'Paste a share string (hydra-player1-…)',
  ...
  common: {
    ...
    updaterInstallHint: 'Close Hydra and run the installer manually.',
  ...
  settings: {
    ...
    testingBtn: 'Testing…',
    serverCompatible: 'Built for Navidrome. Other Subsonic-compatible servers (Gonic, Airsonic, etc.) may work with reduced functionality, since Hydra uses many Navidrome-specific API endpoints.',
    ...
    lfmConnectDesc: 'Connect your Last.fm account to enable scrobbling and Now Playing updates directly from Hydra — no Navidrome configuration required.',
    lfmOpenBrowser: 'A browser window will open. Authorise Hydra on Last.fm, then click the button below.',
    ...
    offlineDirDesc: 'Storage location for tracks you make available offline within Hydra.',
    ...
    audioOutputDeviceDesc: 'Select which audio device Hydra plays through. Changes take effect immediately and restart the current track.',
    ...
    showTrayIconDesc: 'Display the Hydra icon in the system notification area / menu bar.',
    minimizeToTray: 'Minimize to Tray',
    minimizeToTrayDesc: 'When closing the window, keep Hydra running in the system tray instead of quitting.',
    ...
    globalShortcutsNote: 'Work system-wide even when Hydra is in the background. Requires Ctrl, Alt, or Super as a modifier.',
    shortcutClear: 'Clear',
  ...
  help: {
    ...
    a1: 'Hydra works with any Subsonic-compatible server: Navidrome, Gonic, Subsonic, Airsonic, and others. Navidrome is the recommended choice.',
    q2: 'How do I connect to my server?',
    a2: 'Open Settings and click "Add Server". Enter the server URL (e.g. 192.168.1.100:4533), your username, and password. Hydra tests the connection before saving — nothing is stored if the connection fails.',
    ...
    a12: 'Settings → Theme. Choose Hydra Default or a large selection of themes across Hydra Themes, Mediaplayer, Operating Systems, Games, Movies, Series, Social Media, and Open Source Classics (Catppuccin, Nord, Gruvbox, Nightfox).',
    ...
    a16: 'Hydra scrobbles directly to Last.fm — no Navidrome configuration needed. Connect your Last.fm account in Settings → Last.fm and enable scrobbling there.',
    ...
    a20: 'Hydra is available as .deb (Ubuntu/Debian), .rpm (Fedora/RHEL), and via AUR on Arch/CachyOS where packaging is configured. There is no AppImage. If audio is missing, ensure PipeWire or PulseAudio is running.',
    ...
    q30: 'Does Hydra show lyrics?',
    a30: 'Yes. Click the microphone icon in the player bar to open the Lyrics tab in the queue panel. Hydra fetches lyrics automatically from LRCLIB. If synced lyrics are available, the active line is highlighted and scrolls in real time as the song plays. If only plain text is available, the full text is shown statically.',
    q31: 'Can I customize keyboard shortcuts?',
    a31: 'Yes. Settings → Keyboard Shortcuts lets you remap in-app actions (Play/Pause, Next, Previous, Volume Up/Down, Fullscreen, and more) to any key. Settings → Global Shortcuts lets you assign system-wide shortcuts that trigger even when Hydra is in the background.',
    ...
    a35: 'Open any album and click the download icon in the album header. Hydra downloads all tracks in the background. Progress is shown on the button. Once cached, the icon turns green. You can view and remove cached albums in the Offline Library page (sidebar).',
    ...
    a41: 'Settings → Appearance → Auto-Switch Theme: define a day theme and a night theme with start times. Hydra switches automatically at the configured hours. When the scheduler is active, a hint in the theme picker explains why manual theme selection has no immediate effect.',
    ...
    q44: 'What is AutoEQ?',
    a44: 'The 10-band EQ in Settings → Audio includes an AutoEQ lookup. Enter your headphone model and Hydra fetches a correction profile from the AutoEQ database and applies it automatically to the equalizer bands.',
    ...
    a46: 'Hot Cache (Alpha, Settings → Library) preloads the next several tracks in your queue to disk so playback starts instantly with no buffering delay — especially useful on slow or remote servers. Hydra keeps the current track and the next 5 in cache and evicts older entries when the size limit is reached. You can set the maximum cache size and a debounce delay to avoid unnecessary fetches when skipping quickly.',
    ...
    a48: 'When the queue runs out and Repeat is off, Hydra silently appends random tracks from your library so playback never stops. Auto-added tracks appear below a "Added automatically" divider in the queue. Toggle it with the infinity icon in the queue header. You can also restrict auto-added tracks to a specific genre in Settings → Queue.',
    q49: 'Can I control Hydra from the command line?',
    a49: 'Yes — the current CLI binary can remote-control the running app. Examples: hydra-player --player play / pause / next / prev, hydra-player --player volume 75, hydra-player --player seek 15, hydra-player --player mute, hydra-player --player shuffle, hydra-player --player repeat off|all|one, hydra-player --player star / unstar, and hydra-player --player rating 1-5. You can also switch servers, change the audio device, filter the music library, trigger an Instant Mix, and search. Run hydra-player --player --help for the full list. Shell completions for bash and zsh are available via hydra-player completions.',
    ...
    a55: 'Yes. The sync manifest (hydra-player-sync.json) written to the device stores the filename template used during sync. When you open Device Sync on a different OS with a different template, Hydra uses the stored template to correctly detect which files are already on the device — so albums don\'t show as pending when they are already synced.',
    s9: 'Device Sync',
    q56: 'What is Internet Radio?',
    a56: 'The Internet Radio page (sidebar) lets you play any live stream directly in Hydra. Stations are fetched from your Navidrome server — add them there via the Navidrome admin panel, or import a .pls / .m3u file. Playback uses the browser\'s built-in HTML5 audio engine rather than the Rust audio engine, so some EQ and Replay Gain settings do not apply to radio.',
    q57: 'What stream formats does Internet Radio support?',
    a57: 'Hydra supports any format your system\'s audio stack can decode: MP3, AAC, OGG Vorbis, and most HLS streams. The stream is played via HTML5 <audio>, so codec support depends on the platform — MP3 and AAC work everywhere.',
    s10: 'Internet Radio',
    q58: 'Where does Hydra fetch lyrics from?',
    a58: 'Hydra supports three lyrics sources, all configurable in Settings → Lyrics: your Navidrome server (embedded/OpenSubsonic lyrics), LRCLIB (community-contributed synced lyrics), and NetEase Cloud Music (large Chinese and international catalog). You can enable or disable each source and reorder their priority by dragging. Synced lyrics auto-scroll and support click-to-seek; plain-text lyrics are shown statically.',
  ...
  deviceSync: {
    ...
    migrateNoTemplate: 'No legacy filename template found on the device. Migration only applies when the device was synced with a Hydra-compatible version that supported custom templates.',
  ...
  orbit: {
    ...
    helperLink: 'Send this link to your guests. They can paste it anywhere in Hydra with Ctrl+V.',
    ...
    helpIntro: 'Orbit turns Hydra into a shared listening room. A host picks the music; guests listen in sync and can suggest tracks.',
    helpHostOnly: '(host)',
    ...
    helpSec4Body: 'Paste the invite link anywhere in Hydra with Ctrl+V / Cmd+V — the join prompt pops up automatically. Alternatively: "Orbit" → Join a session → paste into the field. If the link points to a server you have an account on, Hydra switches for you. When you have more than one account on that server, a small picker asks which to use.',
