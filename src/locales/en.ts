export const enTranslation = {
  ...
  help: {
    ...
    a49: 'Yes — the current CLI binary can remote-control the running app. Examples: psysonic --player play / pause / next / prev, psysonic --player volume 75, psysonic --player seek 15, psysonic --player mute, psysonic --player shuffle, psysonic --player repeat off|all|one, psysonic --player star / unstar, and psysonic --player rating 1-5. You can also switch servers, change the audio device, filter the music library, trigger an Instant Mix, and search. Run psysonic --player --help for the full list. Shell completions for bash and zsh are available via psysonic completions.',
    ...
    a55: 'Yes. The sync manifest (psysonic-sync.json) written to the device stores the filename template used during sync. When you open Device Sync on a different OS with a different template, Hydra uses the stored template to correctly detect which files are already on the device — so albums don\'t show as pending when they are already synced.',
    s9: 'Device Sync',
