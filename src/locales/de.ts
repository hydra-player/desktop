export const deTranslation = {
  ...
  login: {
    ...
    magicStringPlaceholder: 'Freigabe-String einfügen (hydra-player1-…)',
  ...
  common: {
    ...
    updaterInstallHint: 'Hydra Player schließen und das Installationsprogramm manuell ausführen.',
  ...
  settings: {
    ...
    testingBtn: 'Teste…',
    serverCompatible: 'Für Navidrome entwickelt. Andere Subsonic-kompatible Server (Gonic, Airsonic, …) funktionieren ggf. eingeschränkt, weil Hydra Player viele Navidrome-spezifische API-Endpunkte nutzt.',
    ...
    lfmConnectDesc: 'Verbinde deinen Last.fm Account, um Scrobbling und Now-Playing-Updates direkt aus Hydra Player heraus zu aktivieren — keine Navidrome-Konfiguration erforderlich.',
    lfmOpenBrowser: 'Es öffnet sich ein Browserfenster. Autorisiere Hydra Player auf Last.fm und klicke dann unten auf den Button.',
    ...
    offlineDirDesc: 'Speicherort für Titel, die du innerhalb von Hydra Player offline verfügbar machst.',
    ...
    audioOutputDeviceDesc: 'Wähle das Audiogerät, über das Hydra Player spielt. Änderungen werden sofort übernommen und starten den aktuellen Track neu.',
    ...
    showTrayIconDesc: 'Hydra Player-Icon im System-Tray / in der Menüleiste anzeigen.',
    minimizeToTray: 'Im Tray minimieren',
    minimizeToTrayDesc: 'Beim Schließen des Fensters läuft Hydra Player weiter im System-Tray statt zu beenden.',
    ...
    globalShortcutsNote: 'Funktionieren systemweit, auch wenn Hydra Player im Hintergrund läuft. Mindestens Ctrl, Alt oder Super als Modifier erforderlich.',
  ...
  help: {
    ...
    a1: 'Hydra Player funktioniert mit jedem Subsonic-kompatiblen Server: Navidrome, Gonic, Subsonic, Airsonic und anderen. Navidrome ist die empfohlene Wahl.',
    q2: 'Wie verbinde ich mich mit meinem Server?',
    a2: 'Öffne die Einstellungen und klicke auf "Server hinzufügen". Gib die Server-URL (z. B. 192.168.1.100:4533), Benutzername und Passwort ein. Hydra Player testet die Verbindung vor dem Speichern — bei Fehler wird nichts gespeichert.',
    ...
    a12: 'Einstellungen → Design. Eine große Auswahl an Themes in 8 Gruppen: Hydra Player Themes, Mediaplayer, Betriebssysteme, Spiele, Filme, Serien, Social Media und Open-Source-Classics (Catppuccin, Nord, Gruvbox, Nightfox).',
    ...
    a16: 'Hydra Player scrobbled direkt zu Last.fm — keine Navidrome-Konfiguration nötig. Last.fm-Konto in Einstellungen → Last.fm verbinden und Scrobbling dort aktivieren.',
    ...
    a20: 'Hydra Player ist als .deb (Ubuntu/Debian), .rpm (Fedora/RHEL) und über AUR für Arch/CachyOS verfügbar (yay -S hydra-player). Es gibt kein AppImage. Falls kein Ton kommt, sicherstellen, dass PipeWire oder PulseAudio läuft.',
    ...
    q30: 'Zeigt Hydra Player Songtexte an?',
    a30: 'Ja. Klick auf das Mikrofon-Icon in der Playerleiste öffnet den Lyrics-Tab im Queue-Panel. Hydra Player lädt Texte automatisch von LRCLIB. Sind synchronisierte Lyrics verfügbar, wird die aktuelle Zeile hervorgehoben und scrollt in Echtzeit mit. Bei reinen Texten wird der Volltext statisch angezeigt.',
    ...
    a35: 'Album öffnen und auf das Download-Symbol im Album-Header klicken. Hydra Player lädt alle Tracks im Hintergrund herunter. Der Fortschritt wird am Button angezeigt. Nach dem Download wird das Symbol grün. Gecachte Alben können in der Offline-Bibliothek (Seitenleiste) verwaltet und gelöscht werden.',
    ...
    a41: 'Einstellungen → Darstellung → Theme automatisch wechseln: Tagesthema und Nachtthema mit Startzeiten konfigurieren. Hydra Player wechselt automatisch zur eingestellten Uhrzeit. Ist der Scheduler aktiv, erscheint im Theme-Picker ein Hinweis, warum manuelle Änderungen keine sofortige Wirkung haben.',
    ...
    q44: 'Was ist AutoEQ?',
    a44: 'Der 10-Band-EQ in Einstellungen → Audio enthält eine AutoEQ-Suche. Kopfhörermodell eingeben – Hydra Player lädt das Korrekturprofil aus der AutoEQ-Datenbank und überträgt es automatisch auf die EQ-Bänder.',
    ...
    a46: 'Der Hot Cache (Alpha, Einstellungen → Bibliothek) lädt die nächsten Songs der Warteschlange vorab auf die Festplatte, damit die Wiedergabe ohne Pufferverzögerung startet – besonders nützlich bei langsamen oder entfernten Servern. Hydra Player hält den aktuellen Track und die nächsten 5 im Cache und verdrängt ältere Einträge, wenn das Größenlimit erreicht wird. Cache-Größe und Debounce-Verzögerung sind konfigurierbar.',
    ...
    a48: 'Wenn die Warteschlange leer läuft und Wiederholen deaktiviert ist, fügt Hydra Player automatisch zufällige Tracks aus deiner Bibliothek hinzu, damit die Wiedergabe nie stoppt. Automatisch hinzugefügte Tracks erscheinen unterhalb des Trenners „— Automatisch hinzugefügt —". Umschalten über das Unendlichkeitssymbol im Warteschlangen-Header. In Einstellungen → Warteschlange kann der automatische Nachschub auf ein bestimmtes Genre beschränkt werden.',
    q49: 'Kann ich Hydra Player über die Kommandozeile bedienen?',
    a49: 'Ja – das Hydra Player-Binary kann die laufende App fernsteuern. Beispiele: hydra-player --player play / pause / next / prev, --player volume 75, --player seek 15, --player mute, --player shuffle, --player repeat off|all|one, --player star / unstar, --player rating 1–5. Außerdem: Server wechseln (--player server list / set), Ausgabegerät wählen (--player audio-device list / set), Bibliotheks-Ordner filtern (--player library list / set), Instant Mix starten (--player mix new / append) und Suche (--player search track|album|artist <Suchbegriff>). Vollständige Hilfe: hydra-player --player --help. Shell-Completions für bash und zsh sind über hydra-player completions verfügbar.',
    ...
    a52: 'Einstellungen → Audio → Ausgabegerät. Hydra Player listet alle verfügbaren Ausgaben auf. Auswahl wechselt die Wiedergabe sofort. Den Aktualisierungsbutton daneben nutzen, wenn ein Gerät nach dem App-Start angeschlossen wurde. Unter Linux werden ALSA-Gerätenamen bereinigt angezeigt.',
    q53: 'Was ist Device Sync?',
    a53: 'Device Sync überträgt Alben, Playlists oder ganze Künstler auf einen USB-Stick oder eine SD-Karte. Über „Device Sync" in der Seitenleiste öffnen, einen Zielordner auf dem Laufwerk wählen, Quellen in den Browser-Tabs auswählen und auf „Auf Gerät übertragen" klicken. Hydra Player lädt die Tracks vom Server und legt sie in der durch die Dateinamen-Vorlage definierten Ordnerstruktur ab.',
    ...
    a55: 'Ja. Das auf dem Gerät gespeicherte Manifest (hydra-player-sync.json) enthält die beim Sync verwendete Dateinamen-Vorlage. Wird Device Sync auf einem anderen Betriebssystem mit anderer Vorlage geöffnet, erkennt Hydra Player die bereits vorhandenen Dateien korrekt – Alben werden nicht fälschlich als ausstehend angezeigt.',
    s9: 'Device Sync',
    q56: 'Was ist Internet Radio?',
    a56: 'Die Internetradio-Seite (Seitenleiste) ermöglicht die Wiedergabe beliebiger Livestreams direkt in Hydra Player. Sender werden vom Navidrome-Server bezogen – dort über das Admin-Panel hinzufügen oder eine .pls- / .m3u-Datei importieren. Die Wiedergabe erfolgt über HTML5-Audio, nicht den Rust-Audio-Engine – einige EQ- und Replay-Gain-Einstellungen gelten daher nicht für Radio.',
    q57: 'Welche Stream-Formate unterstützt Internet Radio?',
    a57: 'Hydra Player unterstützt alle Formate, die der Audio-Stack des Systems dekodieren kann: MP3, AAC, OGG Vorbis und die meisten HLS-Streams. Da HTML5-Audio verwendet wird, hängt die Codec-Unterstützung von der Plattform ab – MP3 und AAC funktionieren überall.',
    s10: 'Internet Radio',
    q58: 'Woher bezieht Hydra Player Songtexte?',
    a58: 'Hydra Player unterstützt drei Lyrics-Quellen, alle konfigurierbar in Einstellungen → Lyrics: Navidrome-Server (eingebettete/OpenSubsonic-Lyrics), LRCLIB (community-gepflegte synchronisierte Texte) und NetEase Cloud Music (umfangreicher internationaler Katalog). Jede Quelle kann aktiviert/deaktiviert und per Drag & Drop priorisiert werden. Synchronisierte Lyrics scrollen automatisch und unterstützen Klick-zum-Springen.',
  ...
  deviceSync: {
    ...
    migrateNoTemplate: 'Keine alte Namensvorlage auf dem Gerät gefunden. Migration ist nur möglich, wenn der Stick mit einer Hydra Player-Version synchronisiert wurde, die konfigurierbare Vorlagen unterstützte.',
  ...
  orbit: {
    ...
    helperLink: 'Schicke diesen Link deinen Gästen. Sie fügen ihn mit Strg+V irgendwo in Hydra Player ein.',
    ...
    helpIntro: 'Orbit macht aus Hydra Player einen gemeinsamen Hörraum. Der Host bestimmt die Musik; Gäste hören synchron mit und können Songs vorschlagen.',
    helpHostOnly: '(Host)',
    ...
    helpSec4Body: 'Einladungslink irgendwo in Hydra Player mit Ctrl+V / Cmd+V einfügen — der Beitritt-Dialog erscheint automatisch. Alternativ: „Orbit" → Session beitreten → Link ins Feld pasten. Wenn der Link auf einen Server zeigt, für den du einen Account hast, wechselt Hydra Player automatisch. Bei mehreren Accounts auf demselben Server fragt ein kleiner Picker, welcher genutzt werden soll.',
