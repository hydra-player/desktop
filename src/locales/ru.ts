...
export const ruTranslation = {
  ...
  help: {
    ...
    a16:
      'Hydra Player шлёт прослушивания напрямую на Last.fm — в Navidrome ничего настраивать не нужно. Подключите аккаунт в настройках.',
    ...
    a37: 'Hydra Player поддерживает оценки от 1 до 5 звёзд через OpenSubsonic API (требует Navidrome ≥ 0.53). Треки оцениваются в списке треков альбома или плейлиста, в контекстном меню или прямо в панели плеера. Альбомы — на странице альбома, исполнители — на странице исполнителя.',
    ...
    a41: 'Настройки → Внешний вид → Авто-смена темы: задайте дневную и ночную тему со временем начала. Hydra Player переключается автоматически в указанное время.',
    ...
    a44: '10-полосный эквалайзер в Настройки → Аудио включает поиск AutoEQ. Введите модель наушников — Hydra Player автоматически загрузит и применит профиль коррекции.',
    ...
    a46: 'Горячий кэш (Alpha, Настройки → Библиотека) предзагружает следующие треки очереди на диск для мгновенного воспроизведения. Hydra Player хранит текущий трек и следующие 5, вытесняя старые записи при достижении лимита.',
    ...
    a48: 'Когда очередь заканчивается и повтор отключён, Hydra Player автоматически добавляет случайные треки, чтобы воспроизведение не прерывалось. Автодобавленные треки отображаются под разделителем «— Добавлено автоматически —».',
    q49: 'Можно ли управлять Hydra Player через командную строку?',
    a49: 'Да. Примеры: hydra-player --player play / pause / next / prev, --player volume 75, --player seek 15, --player mute, --player shuffle, --player repeat off|all|one. Также: переключение сервера, аудиоустройства, фильтрация библиотеки и поиск. Полная справка: hydra-player --player --help.',
    ...
    a52: 'Настройки → Аудио → Устройство вывода. Hydra Player показывает все доступные выходы. Выбор вступает в силу немедленно.',
    ...
    a55: 'Да. Манифест на устройстве содержит использованный шаблон. На другой ОС Hydra Player корректно распознаёт уже существующие файлы.',
    s9: 'Device Sync',
    q56: 'Что такое интернет-радио?',
    a56: 'Страница интернет-радио позволяет воспроизводить прямые трансляции в Hydra Player. Станции берутся с сервера Navidrome. Воспроизведение использует HTML5-аудио.',
    ...
    s10: 'Интернет-радио',
    q58: 'Откуда Hydra Player берёт тексты песен?',
