export const zhTranslation = {
  ...
  help: {
    ...
    s1: '入门指南',
    q1: '支持哪些服务器？',
    a1: 'Hydra Player 可与任何 Subsonic 兼容服务器配合使用：Navidrome、Gonic、Subsonic、Airsonic 等。推荐使用 Navidrome。',
    q2: '如何连接服务器？',
    a2: '打开设置并点击"添加服务器"。输入服务器地址（如 192.168.1.100:4533）、用户名和密码。Hydra Player 会在保存前测试连接 — 如果连接失败则不会存储任何信息。',
    q3: '可以使用多个服务器吗？',
    ...
    s4: '设置',
    q12: '如何更改主题？',
    a12: '设置 → 主题。从 8 个分组中的大量主题中选择：Hydra Player 主题、媒体播放器、操作系统、游戏、电影、电视剧、社交媒体，以及开源经典（Catppuccin、Nord、Gruvbox、Nightfox）。',
    q13: '如何更改语言？',
    ...
    s5: '歌曲记录',
    q16: '歌曲记录如何工作？',
    a16: 'Hydra Player 直接向 Last.fm 发送记录 — 无需配置 Navidrome。在 设置 → Last.fm 中连接您的 Last.fm 账户并启用记录功能。',
    q17: '何时发送记录？',
    ...
    q20: 'Linux 上没有音频。',
    a20: 'Hydra Player 提供 .deb（Ubuntu/Debian）、.rpm（Fedora/RHEL）版本，以及通过 AUR 在 Arch/CachyOS 上安装（yay -S hydra-player）。没有 AppImage。如果没有音频，请确保 PipeWire 或 PulseAudio 正在运行。',
    q21: '应用在 Linux 上显示黑屏。',
    ...
    q30: 'Hydra Player 显示歌词吗？',
    a30: '是的。点击播放器栏的麦克风图标可在队列面板中打开歌词标签。Hydra Player 自动从 LRCLIB 获取歌词。如果可用同步歌词，活动行会高亮并随歌曲实时滚动。如果只有纯文本歌词，则静态显示完整文本。',
    q31: '可以自定义键盘快捷键吗？',
    a31: '可以。设置 → 键盘快捷键 允许您重新映射应用内操作（播放/暂停、下一首、上一首、音量增/减、全屏等）到任意按键。设置 → 全局快捷键 允许您分配即使 Hydra Player 在后台也能触发的系统级快捷键。',
    q32: '可以更改字体吗？',
    ...
    q35: '如何缓存专辑以供离线使用？',
    a35: '打开任意专辑并点击专辑标题栏的下载图标。Hydra Player 会在后台下载所有曲目。按钮上会显示进度。缓存完成后，图标变为绿色。您可以在离线音乐库页面（侧边栏）查看和移除缓存的专辑。',
    q36: '离线缓存可以使用多少存储空间？',
    a36: '您可以在 设置 → 音乐库 中设置最大缓存大小。达到限制时，专辑页面会显示警告横幅。您可以从离线音乐库中删除单个专辑以释放空间。',
    q37: '如何对歌曲、专辑和艺术家评分？',
    a37: 'Hydra Player 通过 OpenSubsonic API 支持 1–5 星评分（需要 Navidrome ≥ 0.53）。可在专辑或播放列表曲目列表、右键菜单或播放器栏中对歌曲评分。专辑在详情页评分，艺术家在艺术家页评分。',
    q38: '可以删除或更改评分吗？',
    ...
    q41: '什么是主题调度器？',
    a41: '设置 → 外观 → 自动切换主题：设置日间主题和夜间主题及开始时间，Hydra Player 将在指定时间自动切换主题。',
    q42: '可以调整界面缩放吗？',
    ...
    q46: '什么是热缓存？',
    a46: '热缓存（Alpha，设置 → 音乐库）将队列中后续曲目预加载到磁盘，实现即时播放，对于慢速或远程服务器尤为有用。Hydra Player 保留当前曲目和后续 5 首，达到大小限制时淘汰旧条目。',
    q47: '可以将播放列表缓存到离线吗？',
    a47: '可以。在播放列表详情中点击下载图标，所有曲目将在后台下载。完全缓存后图标变为红色垃圾桶；再次点击即可从离线缓存中删除该播放列表。',
    q48: '什么是无限队列？',
    a48: '当队列播完且关闭了重复播放时，Hydra Player会自动添加随机曲目，确保播放不会中断。自动添加的曲目显示在"— 自动添加 —"分隔符下方。',
    q49: '可以通过命令行控制Hydra Player吗？',
    a49: '可以。示例：hydra-player --player play / pause / next / prev，--player volume 75，--player seek 15，--player mute，--player shuffle，--player repeat off|all|one。还可以切换服务器、音频设备、筛选音乐库和搜索。完整帮助：hydra-player --player --help。',
    q50: '如何管理播放列表？',
    ...
    q52: '如何更改音频输出设备？',
    a52: '设置→音频→输出设备。Hydra Player列出所有可用输出，选择后立即生效。若设备在应用启动后连接，可点击刷新按钮更新列表。',
    q53: '什么是设备同步？',
    ...
    q55: '设备同步支持跨平台使用吗？',
    a55: '支持。设备上的清单文件包含同步时使用的模板。在不同系统上，Hydra Player能正确识别已存在的文件。',
    s9: 'Device Sync',
    q56: '什么是网络电台？',
    a56: '网络电台页面可在Hydra Player中播放任何直播流。电台来自Navidrome服务器，可通过管理面板添加或导入.pls/.m3u文件。播放使用HTML5音频。',
    q57: '网络电台支持哪些流格式？',
    a57: 'MP3、AAC、OGG Vorbis及大多数HLS流。MP3和AAC在所有平台均可使用。',
    s10: '网络电台',
    q58: 'Hydra Player从哪里获取歌词？',
